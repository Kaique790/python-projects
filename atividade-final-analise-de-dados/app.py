...
df = pd.read_csv("funcionarios-empresa - EMPRESA.csv")
pd.DataFrame(df)

df = df.dropna(how="all", axis=1)

def formatBRL (column):
    df[column] = df[column].str.replace("R\$ ?", "", regex=True)
    df[column] = df[column].str.replace(".", "")
    df[column] = df[column].str.replace(",", ".")
    df[column] = pd.to_numeric(df[column])

formatBRL("Salário Líquido")
formatBRL("Hora Extra (Total.)")
formatBRL("Imposto de Renda R$")
formatBRL("Salário Bruto")
...
if selected_option == options[3]:
    ...
    higher_workers = df[df["Gratificação R$"] == max_gratification]["Nome Completo"]
    min_workers = df[df["Gratificação R$"] == min_gratification]["Nome Completo"]
...
if(selected_option == options[4]):
    ...
    higher_worker = df[df["Imposto de Renda R$"] == max_tax]["Nome Completo"].iloc[0]
    min_worker = df[df["Imposto de Renda R$"] == min_tax]["Nome Completo"].iloc[0]
